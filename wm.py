...
def process_video(subtitres_whisper, sURL, subtitres_lang, t_video, t_audio):
    ...
    if ("v=" in sURL and sURL[:30] == "https://www.youtube.com/watch?") or (
        "shorts/" in sURL and sURL[:31] == "https://www.youtube.com/shorts/"
    ):
...
def GetSubtitles(first_language_code, yt):
    ...
    languages_text = "Базовые языки:\n" + "\n".join(
        f"   - {code} - {transcript.language}"
        for code, transcript in available_subtitles.items()
    )

    if first_language_code in available_subtitles:
        subtitles_text = "\n".join(
            s1["text"]
            for s1 in available_subtitles[first_language_code].fetch()
        )
    elif first_language_code in {
        tr_l["language_code"]
        for tr_l in translatable_subtitles[first_language_code].
        translation_languages
    }:
        subtitles_text = "\n".join(
            s1["text"]
            for s1 in translatable_subtitles[first_language_code]
            .translate(first_language_code)
            .fetch()
        )
    else:
        languages_text += "\nПереводы:\n" + "\n".join(
            f"   - Базовый язык {transcript.language_code}\
             - {transcript.language}. Переводы:\n"
            + "      - "
            + "\n      - ".join(
                f'{tr_l["language_code"]} - {tr_l["language"]}'
                for tr_l in transcript.translation_languages
            )
            for transcript in translatable_subtitles.values()
        )
        return f"Указанный код языка не найден. Возможно выбрать\
         указанные ниже языки:\n{languages_text}"
...
def GetTextFromVideoYt(yt):
    try:
        ...
        return "Видео имеет возрастные ограничения, и к нему невозможно \
                получить доступ без входа в систему.. \nПопробуйте выбрать \
                другие параметры работы с видео. \nНапример: \
                    Распознать загруженное аудио/видео"
    except Exception:
...
def process_summarize(sIn):
    if sIn != "" and not (sIn is None) and sIn != "...":
        ...
        summarized_text = summarizer(
            parser.document, sentences_count=10
        )  # Меняем число предложений
...
if __name__ == "__main__":
    ...
    t_subtitres_whisper = st.radio(
        ...
        [
            "Использовать субтитры YouTube",
            "Распознать аудио с YouTube",
            "Распознать загруженное аудио/видео",
        ],
        on_change=st.session_state.clear,
    )

    t_subtitres_lang = st.selectbox("Язык субтитров", ("RU", "EN")).lower()

    t_video = st.file_uploader(
        "Выберете видео файл для распознавания",
        accept_multiple_files=False,
        on_change=st.session_state.clear,
    )
    t_audio = st.file_uploader(
        "Выберете аудио файл для распознавания",
        accept_multiple_files=False,
        on_change=st.session_state.clear,
    )
    ...
    def click_button0():
        if not t_sURL and not t_video and not t_audio:
            st.session_state["result0"] = (
                "Укажите ссылку на видео или загрузите видео/аудио файл"
            )
            return
        if (
            t_subtitres_whisper == "Использовать субтитры YouTube"
            and not t_subtitres_lang
        ):
            st.session_state["result0"] = "Укажите язык субтитров"
            return
        st.session_state["result0"] = process_video(
            t_subtitres_whisper, t_sURL, t_subtitres_lang, t_video, t_audio
        )
    ...
    with col3:
        st.download_button(
            label=":inbox_tray:",
            data=st.session_state["result0"],
            mime="text/plain",
            key="000",
            file_name=f"{button_name0[9:].capitalize()}.txt",
        )

    def click_button1():
        if (
            "result0" not in st.session_state
            or st.session_state["result0"] == ""
            or st.session_state["result0"]
            == "Укажите ссылку на видео или загрузите видео/аудио файл"
            or st.session_state["result0"] == "Укажите язык субтитров"
        ):
            st.session_state["result1"] = "Сначала получите\
                 субтитры из видео/аудио"
            return
        st.session_state["result1"] = \
            process_summarize(st.session_state["result0"])
    ...
    with col3:
        st.download_button(
            ...
            mime="text/plain",
            key="111",
            file_name=f"{button_name1[9:].capitalize()}.txt",
        )
    ...
    def click_button2():
        if (
            ...
            st.session_state["result2"] = \
                "Сначала получите субтитры из видео/аудио"
            return
