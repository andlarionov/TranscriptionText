...
def test_video():
    # распознаём видео файл (в папке с проверяемым фалом д/б ещё файл Savvateev.mp4)
    subtitres_whisper = 'Распознать загруженное аудио/видео'
    sURL = 'https://www.youtube.com/watch?v=6EsCI3CbmTk'
    subtitres_lang = 'ru'
...
def test_process_summarize_valid():
    ...
    input_text = """
    Это пример текста, который предназначен для проверки функции суммаризации. Текст содержит несколько предложений,
    которые описывают функциональность и позволяют убедиться в корректности работы алгоритма. Суммаризация текста - важная
    задача, которая позволяет выделить наиболее значимые части длинного текста и представить их в сокращенном виде.
    Функция должна корректно выбирать ключевые предложения, которые наиболее полно отражают содержание всего текста.
    """
